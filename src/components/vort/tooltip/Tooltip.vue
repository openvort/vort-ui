...
defineOptions({ name: "VortTooltip" });
...
const updateArrowPosition = (placement: FloatingPlacement, triggerRect: DOMRect, floatingWidth: number, floatingHeight: number) => {
    ...
    const getHorizontalOffset = () => Math.min(Math.max(triggerRect.width / 2, 12), floatingWidth - 12);
    const getVerticalOffset = () => Math.min(Math.max(triggerRect.height / 2, 12), floatingHeight - 12);
...
// 背景颜色 - 默认白色，dark 模式为暗色
const bgColor = computed(() => {
    if (props.color) return props.color;
    return props.dark ? "#222" : "#fff";
});

// 文字颜色 - 根据背景自动调整
const textColor = computed(() => {
    if (props.dark || props.color) return "#fff";
...
const floatingStyle = computed(() => ({
    ...
    "--tooltip-bg": bgColor.value,
    "--tooltip-text": textColor.value,
    ...props.overlayStyle
}));
...
<template>
    ...
    <Teleport v-if="shouldRenderTeleport" :to="popupContainer">
        <Transition name="vort-tooltip" appear @after-leave="handleAfterLeave">
            <div
                v-if="visible && (title || $slots.default)"
                ref="floatingRef"
                :class="['vort-tooltip', props.overlayClass]"
                :style="floatingStyle"
...
.vort-tooltip {
    ...
    border-radius: var(--vort-border-radius-sm, 6px);
    pointer-events: auto;
    box-shadow:
...
.vort-tooltip-enter-active {
    animation: vortTooltipIn 0.1s cubic-bezier(0.08, 0.82, 0.17, 1);
    transform-origin: var(--transform-origin, center);
}

.vort-tooltip-leave-active {
    animation: vortTooltipOut 0.1s cubic-bezier(0.78, 0.14, 0.15, 0.86);
    transform-origin: var(--transform-origin, center);
}

@keyframes vortTooltipIn {
    0% {
        opacity: 0;
        transform: scale(0.8);
    }
...
.vort-tooltip-arrow.arrow-bottom {
    transform: translateX(-50%);
    height: 8px;
}

.vort-tooltip-arrow.arrow-bottom::before {
    left: 50%;
    top: -2px;
...
.vort-tooltip-arrow.arrow-top {
    transform: translateX(-50%);
    height: 8px;
}

.vort-tooltip-arrow.arrow-top::before {
    left: 50%;
    bottom: -2px;
...
.vort-tooltip-arrow.arrow-right {
    transform: translateY(-50%);
    width: 8px;
    height: 16px;
}

.vort-tooltip-arrow.arrow-right::before {
    top: 50%;
    left: -2px;
...
.vort-tooltip-arrow.arrow-left {
    transform: translateY(-50%);
    width: 8px;
    height: 16px;
}

.vort-tooltip-arrow.arrow-left::before {
    top: 50%;
    right: -2px;
...
.vort-tooltip-inner {
    font-size: 14px;
    color: var(--tooltip-text, rgba(0, 0, 0, 0.88));
    line-height: 1.5;
